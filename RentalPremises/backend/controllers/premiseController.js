...
module.exports = {
    async getPremises(req, res) {
		try {
			...
			const premises = req.query.id ? await premisesDb.findNotRentedPremisesByType(parseInt(req.query.id)) : await premisesDb.findNotRentedPremises();
	
			for(const premise of premises) 
				premise.Image = imgSrc + `${premise.Image ? premise.Image.toString('base64') : null}`;
	...
	async getMyPremises(req, res) {
		try {
			...
			const premises = await premisesDb.findPremisesByOwner(parseInt(userData.id))
	...
	async getMyRentedPremises(req, res) {
		try {
			...
			if (req.query.ID) return res.status(200).send(await premisesDb.findRentedPremisesByOwner(parseInt(userData.id)));
		} catch (error) {
	...
	async editPremise(req, res) {
		try {
			...
			if (!await premisesDb.findPremisesByIdAndOwner(parseInt(req.body.id), parseInt(userData.id))) return res.status(403).send('У вас нет прав!');
